  ...
  class ModSecurityAuditFormat < Output
    ...
    def process(tag, es)
      es.each do |time, record|
        ...
        if modsecRecord.key?('auditLogTrailer') and modsecRecord['auditLogTrailer'].key?('messages')
          modsecRecord['auditLogTrailer']['messages'].each { |log|
            ...
            record['tags'] = [] if record['tags'].nil?
    ...
    def processSectionB(section)
      ...
      if section =~ /.+\n(?m).+/
        ...
        hash['raw_requestHeaders'].split(/\n/).each do |header|
          parts = header.split(/:/)
          hash['requestHeaders'][parts[0].strip.downcase] = parts[1].strip
        end
    ...
    def processSectionF(section)
      if section =~ /.+/
        ...
        hash['raw_responseHeaders'].split(/\n/).each do |header|
          parts = header.split(/:/)
          hash['responseHeaders'][parts[0].strip.downcase] = parts[1].strip
        end
